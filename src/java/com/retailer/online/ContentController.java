...
public class ContentController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            s2.execute("SELECT ID,NAME,GROUPID,GROUPNAME,GROUPDETAILS,COALESCE(PPRICE, 0) AS PPRICE,PRICE FROM ITEM WHERE ID = "+recentId+"");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Connection conn = ds.getConnection();
            Statement s2 = conn.createStatement();
            String search_gropname = request.getParameter("search.gropname");
            //int recentId = Integer.parseInt(request.getParameter("recentId"));
            s2.execute("SELECT ID,NAME,GROUPID,GROUPNAME,GROUPDETAILS,COALESCE(PPRICE, 0) AS PPRICE,PRICE FROM ITEM WHERE GROUPNAME LIKE '%"+search_gropname+"%'");
            ResultSet r2 = s2.getResultSet();
            while (r2.next()) {
                item = new Item();
                item.setId(Integer.parseInt(r2.getString("ID")));
                ...
                item.setPprice(Integer.parseInt(r2.getString("PPRICE")));
                item.setPrice(Integer.parseInt(r2.getString("PRICE")));
                listofItem4.setItems(item);
            }
            conn.close();
        } catch (Exception ex) {
        }       
        session.setAttribute("listofItem1", listofItem4);
        response.sendRedirect("productList.jsp");        
        return;
    }
